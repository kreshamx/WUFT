...
CREATE TABLE [dbo].[NLog_Error](
 ...
 CONSTRAINT [PK_NLog_ErrorId] PRIMARY KEY CLUSTERED 
(
	[ErrorId] ASC
...
ALTER TABLE [dbo].[NLog_Error] ADD 
    CONSTRAINT [DF_NLog_Error_ErrorId] DEFAULT (NEWID()) FOR [ErrorId]
GO

ALTER TABLE [dbo].[NLog_Error] ADD  CONSTRAINT [DF_NLogError_time_stamp]  DEFAULT (getdate()) FOR [DateTimeLogged]
GO
...
CREATE VIEW [dbo].[AllLogs_V]
AS
SELECT		[ErrorId],[Application], [LogSource] = 'NLog', [Host], [Type], [Source], [Message], [Level], [User], [AllXml], [DateTimeLogged]
FROM		[ISEToolLogs].[dbo].[NLog_Error]
UNION ALL
SELECT		[ErrorId],[Application], [LogSource] = 'ELMAH', [Host], [Type], [Source], [Message], [Level] = 'Error', [User], [AllXml], [TimeUtc] as DateTimeLogged
FROM		[ISEToolLogs].[dbo].[ELMAH_Error]
